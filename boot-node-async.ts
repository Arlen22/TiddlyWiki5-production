						...
						if (typeof fieldInfo === "string" || $tw.utils.isArray(fieldInfo)) {
							...
							var newValue: any = (() => {
								switch (fieldInfo.source) {
									...
									case "created":
										return obs_stat()(pathname).map(([err, stat]) => stat.birthtime);
									case "modified":
										return obs_stat()(pathname).map(([err, stat]) => stat.mtime);
								}
							})();
	...
	function loadTiddlersNode() {
		return Observable.merge(
			...
			$tw.loadTiddlersFromPath($tw.boot.bootPath)
				.do(tiddlerFile => $tw.wiki.addTiddlers(tiddlerFile.tiddlers)),
			// Load the core tiddlers
			$tw.loadPluginFolder($tw.boot.corePath)
				.do(pluginFolder => $tw.wiki.addTiddler(pluginFolder)),
			// Load the tiddlers from the wiki directory
			($tw.boot.wikiPath) ?
				$tw.loadWikiTiddlers($tw.boot.wikiPath).do(wikiInfo => $tw.boot.wikiInfo = wikiInfo)
				: Observable.empty()
		).ignoreElements();
	}
