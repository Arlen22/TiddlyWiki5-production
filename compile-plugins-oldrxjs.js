...
const TWCompile = process.argv[3];
const TWSource = process.argv[2];
if (!TWCompile || !TWSource) {
	console.log("source destination required")
	return;
}


const oldFolder = path.resolve(TWSource);
const newFolder = path.resolve(TWCompile);

if (fs.existsSync(newFolder)) fs.rmdirSync(newFolder);
